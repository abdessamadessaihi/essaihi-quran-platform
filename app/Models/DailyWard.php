...
class DailyWard extends Model
{
    public $timestamps = false;

    const UNIT_PAGES = 'pages';
    const UNIT_HIZBS = 'hizbs';
    const UNIT_JUZ   = 'juz';
    const UNIT_AYAHS = 'ayahs';
    ...
    protected $fillable = [
        'user_id','ward_date','target_unit','target_value',
        'actual_value','adherence_pct','is_completed','completed_at',
        'location_type','start_page','end_page',
        'start_surah','end_surah','start_hizb','end_hizb',
        'start_juz','end_juz','notes','khatma_id',
    ];
    ...
    public function complete(float $actualValue): void
    {
        $adherence = $this->target_value > 0
            ? min(100, round(($actualValue / $this->target_value) * 100))
            : 0;

        $this->update([
            'actual_value'  => $actualValue,
            'adherence_pct' => $adherence,
            'is_completed'  => $adherence >= 80,
            'completed_at'  => now(),
        ]);
    }
    ...
    {
        return self::UNIT_LABELS[$this->target_unit] ?? $this->target_unit;
    }
}