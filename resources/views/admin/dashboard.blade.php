...
<div class="page-header" style="margin-bottom:32px">
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap">
    ...
    {{-- 💡 تحسين: جعل أزرار الإدارة تأخذ العرض الكامل بالتساوي على الهاتف --}}
    <div class="admin-header-buttons" style="display:flex;gap:10px;flex-wrap:wrap;">
      ...
      <a href="{{ route('admin.families.index') }}" class="admin-btn"
         style="display:inline-flex;align-items:center;justify-content:center;gap:7px;
                padding:10px 18px;border-radius:11px;
                background:var(--card);border:1.5px solid var(--border);
                font-family:'Tajawal',sans-serif;font-size:13px;
...
{{-- ══ إحصائيات المنصة ══ --}}
{{-- 💡 تحسين: تعديل minmax إلى 140px لكي تترتب الإحصائيات بشكل شبكي ثنائي متناسق جداً على الهاتف بدلاً من التلاصق --}}
<div class="stats-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:16px;margin-bottom:32px">
  @foreach([
  ...
  <div class="stat-card" style="background:{{ $bg }};border:1px solid {{ $border }};
...
{{-- ══ أحدث النشاطات ══ --}}
{{-- 💡 تحسين: تعديل minmax(400px) الكارثي للهاتف إلى minmax(100%, 1fr) لمنع تمدد الجداول خارج الشاشة --}}
<div class="activities-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%, 400px),1fr));gap:24px">
  
  {{-- المستخدمين الجدد --}}
  <div class="admin-card" style="background:var(--card);border:1px solid var(--border);border-radius:20px;overflow:hidden">
    ...
    <div style="padding:0">
      ...
      <div style="padding:16px 24px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:14px">
        ...
        <div style="flex:1;min-width:0">
          <p style="font-size:13.5px;font-weight:700;color:var(--text);margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $u->name }}</p>
  ...
  <div class="admin-card" style="background:var(--card);border:1px solid var(--border);border-radius:20px;overflow:hidden">
    ...
    <div style="padding:0">
      ...
      <div style="padding:16px 24px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:14px">
        ...
        <div style="flex:1;min-width:0">
          <p style="font-size:13.5px;font-weight:700;color:var(--text);margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $f->name }}</p>
          <p style="font-size:11.5px;color:var(--text-m);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">بواسطة: {{ $f->creator->name ?? 'غير معروف' }}</p>
        </div>
        <div style="flex-shrink:0">
          <span style="font-size:10.5px;padding:3px 10px;border-radius:100px;
...
@push('styles')
<style>
/* ══ إدارة استجابة شاشات الجوال بشكل ذكي ومنظم ══ */
@media (max-width: 640px) {
  /* جعل أزرار الهيدر الإداري تأخذ مساحة متساوية وعرض كامل بشكل منظم */
  ...
  /* تصغير حجم خط الأرقام الكبيرة في الإحصائيات لتناسب الهواتف الصغيرة جداً */
  ...
  /* خلق تباعد ذكي (Ecart) أسفل كل جدول أو كرت لمنع الالتصاق التام عند الاصطفاف */
  .admin-card {
