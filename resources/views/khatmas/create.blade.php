  ...
  <form method="POST" action="{{ route('khatmas.store') }}"
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
        ...
        <label style="position:relative;cursor:pointer">
          ...
          <div :style="type === '{{ $val }}'
               ...
               style="padding:16px 12px;border-radius:14px;
                      border:2px solid;text-align:center;
                      transition:all .18s">
            <span style="font-size:24px;display:block;margin-bottom:6px">
              {{ $icon }}
            </span>
            ...
            <p style="font-size:11px;color:var(--text-m);line-height:1.4">
              {{ $desc }}
            </p>
          </div>
          <div x-show="type === '{{ $val }}'"
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        <div>
          <label class="field-label">
            عنوان الختمة <span style="color:#ef4444">*</span>
          </label>
          <input type="text" name="title"
                 value="{{ old('title') }}"
                 placeholder="مثال: ختمة رمضان ١٤٤٦"
                 class="field-input"
    ...
    <div class="form-section">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        @foreach([
          [true,  '🤖', 'توزيع تلقائي',
           'يوزّع النظام الأجزاء تلقائياً بالتساوي على المشاركين'],
          [false, '✋', 'اختيار يدوي',
           'يختار كل عضو الجزء الذي يريد قراءته بنفسه'],
        ] as [$val,$icon,$label,$desc])
        <label style="cursor:pointer">
          <input type="hidden" name="auto_distribute" :value="autoDistribute"/>
          <div @click="autoDistribute = {{ $val ? 'true' : 'false' }}"
               :style="autoDistribute === {{ $val ? 'true' : 'false' }}
                 ? 'border-color:#059669;background:#ecfdf5'
                 : 'border-color:var(--border);background:var(--card)'"
               style="padding:18px;border-radius:14px;border:2px solid;
                      text-align:center;transition:all .18s;cursor:pointer">
            <span style="font-size:26px;display:block;margin-bottom:8px">
              {{ $icon }}
            </span>
            <p style="font-size:13px;font-weight:700;
            ...
            <p style="font-size:11.5px;color:var(--text-m);line-height:1.6">
              {{ $desc }}
            </p>
          </div>
        </label>
