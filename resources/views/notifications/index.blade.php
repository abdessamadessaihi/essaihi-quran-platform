...
<div class="page-header" style="margin-bottom:32px">
  ...
  <p class="page-subtitle">ابقَ على اطلاع بكل ما يتعلق بحفظك وورادك ومسيرتك القرآنية</p>
</div>
  ...
  <div style="display:flex;flex-direction:column;gap:20px">
    ...
    <div class="card">
      ...
      <div style="padding:18px;display:flex;flex-direction:column;gap:14px">
        @php
          ...
          $typeCounts  = $notifications->groupBy('type');
        @endphp

        {{-- شريط التقدم --}}
        <div>
          <div style="display:flex;justify-content:space-between;
                      font-size:11.5px;color:var(--text-m);margin-bottom:6px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          @foreach([
            ...
            ['هذا الأسبوع',
             $notifications->filter(fn($n) => $n->created_at->isCurrentWeek())->count(),
          ...
          <div style="padding:12px;border-radius:10px;
            ...
            <p style="font-size:10.5px;color:var(--text-m);
                      margin-top:4px;font-weight:600">{{ $lbl }}</p>
          </div>
          @endforeach
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div style="padding:14px;display:flex;flex-direction:column;gap:10px">
        @foreach([
          ['ward',         '🌙', 'الورد اليومي',       'إشعارات الورد والتذكير اليومي'],
          ['memorization', '📖', 'الحفظ والمراجعة',    'تذكيرات الحفظ الجديد والمراجعات'],
          ['streak',       '🔥', 'سلاسل الأيام',       'تحقيق وحماية سلسلتك اليومية'],
          ['badge',        '🏅', 'الأوسمة والإنجازات', 'حصولك على وسام جديد أو إنجاز'],
          ['family',       '👨‍👩‍👧', 'تحديثات العائلة',  'أنشطة وتحديثات أفراد عائلتك'],
        ] as [$key, $icon, $label, $desc])
        <div style="display:flex;align-items:flex-start;gap:10px;
                    padding:10px;border-radius:10px;background:var(--bg)">
          <span style="font-size:18px;flex-shrink:0">{{ $icon }}</span>
          <div>
            <p style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:2px">
              {{ $label }}
            </p>
            <p style="font-size:11px;color:var(--text-m);line-height:1.5">
              {{ $desc }}
            </p>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <div style="padding:14px;display:flex;flex-direction:column;gap:8px">
        @foreach([
          [route('ward.index'),          '🌙', 'الورد اليومي'],
          [route('memorizations.index'), '📖', 'صفحة الحفظ'],
          [route('revisions.index'),     '🧠', 'المراجعة'],
          [route('leaderboard'),         '🏆', 'لوحة الشرف'],
        ] as [$url, $icon, $label])
        <a href="{{ $url }}"
          ...
          <svg width="14" height="14" fill="none" stroke="currentColor"
               stroke-width="2" viewBox="0 0 24 24"
               style="margin-right:auto;color:var(--text-m)">
...
.notif-icon-wrap {
  ...
  font-size:20px;transition:opacity .2s;
}
...
.empty-bell {
  ...
  background:linear-gradient(135deg,#f4f7f5,#e8f5ef);
...
@push('scripts')
<script>
// Helpers: resolve icon class, icon emoji, title, desc, channel label
// (Blade helpers are used via PHP functions defined below)
</script>
@endpush